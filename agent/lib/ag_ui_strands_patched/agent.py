...
class StrandsAgent:
    ...
    def __init__(
        ...
        # Track tool names per thread for cache invalidation on mode change
    ...
    async def run(self, input_data: RunAgentInput) -> AsyncIterator[Any]:
        ...
        # Determine tools for this request (mode-based filtering)
        request_tools = self._tools
        if self.config.tools_provider:
            try:
                provided = self.config.tools_provider(input_data)
                if provided is not None:
                    request_tools = provided
        ...
        cached_tool_names = self._tool_names_by_thread.get(thread_id)

        if thread_id not in self._agents_by_thread or cached_tool_names != tool_names:
            if cached_tool_names and cached_tool_names != tool_names:
                logger.info(f"Tools changed for thread {thread_id}, recreating agent")
            session_manager = None
            if hasattr(self.config, 'session_manager_provider') and self.config.session_manager_provider:
                try:
                    session_manager = self.config.session_manager_provider(input_data)
                except Exception as e:
                    logger.warning(f"session_manager_provider failed: {e}")
            ...
            self._tool_names_by_thread[thread_id] = tool_names
        ...
        try:
            ...
            # Note: We no longer skip tool call events based on this flag.
            # The old behavior suppressed ToolCallStartEvent for ALL tool calls
            # when a tool result was pending, which broke quiz rendering after
            # backend tool calls. Now we always emit tool call events.
            ...
            if input_data.messages:
                ...
                if last_msg.role == "tool":
                    ...
                    pending_tool_call_id = getattr(last_msg, 'tool_call_id', None)
                    logger.debug(
                        f"Has pending tool result detected (no longer skipping events): tool_call_id={pending_tool_call_id}, thread_id={input_data.thread_id}"
                    )
            ...
            try:
                async for event in agent_stream:
                    ...
                    if event.get("complete") or event.get("force_stop"):
                        ...
                        # Surface force_stop reason if present
                        force_stop_reason = event.get("force_stop_reason", "")
                        if force_stop_reason:
                            logger.error(f"Agent force-stopped: {force_stop_reason}")
                            yield RunErrorEvent(
                                type=EventType.RUN_ERROR,
                                message=f"Agent error: {force_stop_reason}",
                                code="FORCE_STOP",
                            )
                        # Surface any error from the complete event
                        complete_data = event.get("complete", {})
                        if isinstance(complete_data, dict):
                            error_msg = complete_data.get("error", "") or complete_data.get("message", "")
                            if error_msg:
                    ...
                    # Handle content block stop - this signals tool input is complete
                        ...
                        if "contentBlockStop" in inner_event:
                            ...
                            if tool_name and tool_use_id:
                                ...
                                if not has_pending_tool_result or True:  # Always emit tool call events
                                    logger.debug(
                                        f"Emitting tool call events for {tool_name} (tool_use_id={tool_use_id}, thread_id={input_data.thread_id})"
                                    )

                                    # For frontend tools: close the current text message
                                    # and start a fresh assistant message for the tool call.
                                    # This prevents defaultApplyEvents from attaching the
                                    # frontend tool call to the wrong message (e.g. a tool
                                    # result message from a preceding backend tool call).
                                    tool_parent_id = message_id
                                    if is_frontend_tool and message_started:
                                        yield TextMessageEndEvent(
                                            type=EventType.TEXT_MESSAGE_END,
                                            message_id=message_id,
                                        )
                                        message_started = False
                                        tool_parent_id = str(uuid.uuid4())

                                    yield ToolCallStartEvent(
                                        type=EventType.TOOL_CALL_START,
                                        tool_call_id=tool_use_id,
                                        tool_call_name=tool_name,
                                        parent_message_id=tool_parent_id,
                                    )

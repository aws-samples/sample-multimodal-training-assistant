...
export class AuthStack extends cdk.NestedStack {
  ...
  constructor(scope: Construct, id: string, props: AuthStackProps) {
    ...
    this.userPool = new cognito.UserPool(this, 'ChatbotUserPool', {
      ...
      removalPolicy: props.resourceSuffix === 'prod'
    ...
    this.userPoolClient = this.userPool.addClient('ChatbotUserPoolClient', {
      authFlows: {
        userPassword: true, // Required for E2E test auth
        userSrp: true,
    ...
    this.authenticatedRole = new iam.Role(this, 'ChatbotIdentityPoolAuthRole', {
      ...
      inlinePolicies: {
          ...
          statements: [
            new iam.PolicyStatement({
              ...
              resources: props.retrievalFunction ?
                ...
                [] // No fallback — retrievalFunction is always passed from MultimediaRagStack
            })
          ]
        }),
        ...
        // Policy 4: AgentCoreAccess - For AgentCore WebSocket streaming
        ...
        // Policy 5: BedrockAccess - KB sync permissions added by parent stack
        // with specific KB ARN (see multimedia-rag-stack.ts)
      }
    });
    ...
    // Store Cognito config in SSM for frontend
    new ssm.StringParameter(this, 'UserPoolClientIdParameter', {
      parameterName: `/multimedia-rag/${props.resourceSuffix}/cognito-user-pool-client-id`,
      stringValue: this.userPoolClient.userPoolClientId,
      description: 'Cognito User Pool Client ID',
    });

    new ssm.StringParameter(this, 'IdentityPoolIdParameter', {
      parameterName: `/multimedia-rag/${props.resourceSuffix}/cognito-identity-pool-id`,

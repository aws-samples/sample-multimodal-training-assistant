...
def _update_file_status(s3_key, status, metadata=None, error_message=None):
    """Update file status in DynamoDB file-status table."""
    table_name = os.environ.get('FILE_STATUS_TABLE')
    if not table_name:
        logger.warning("FILE_STATUS_TABLE not configured, skipping status update")
        return
    ...
    table.update_item(
        ...
        UpdateExpression='SET #st = :status, updatedAt = :updatedAt'
            + (', processedAt = :processedAt' if 'processedAt' in item else '')
            + (', metadata = :metadata' if metadata else '')
            + (', errorMessage = :errorMessage' if error_message else ''),
    ...
    logger.info(f"Updated file status: s3Key={s3_key}, status={status}")


def _extract_file_metadata(result_content):
    """Extract metadata from BDA result for the file status record."""
    raw_metadata = result_content.get('metadata', {})
    modality = raw_metadata.get('semantic_modality', 'UNKNOWN')

    metadata = {'modality': modality}

    if 'number_of_pages' in raw_metadata:
        metadata['pages'] = raw_metadata['number_of_pages']
    if 'duration_millis' in raw_metadata:
        metadata['durationSeconds'] = raw_metadata['duration_millis'] // 1000
...
def lambda_handler(event, context):
    ...
    if event['detail-type'] == 'Bedrock Data Automation Job Succeeded':
        try:
            ...
            input_s3_key = detail['input_s3_object']['name']

            # Get original filename with underscore and extension without dot
            original_filename = f"{os.path.splitext(input_s3_key)[0]}_{os.path.splitext(input_s3_key)[1][1:]}"
            ...
            # Update file status to 'processed' in DynamoDB
            file_metadata = _extract_file_metadata(result_content)
            _update_file_status(input_s3_key, 'processed', metadata=file_metadata)
            ...
            # Attempt to mark file as error in DynamoDB
            try:
                input_s3_key = event.get('detail', {}).get('input_s3_object', {}).get('name', '')
                if input_s3_key:
                    _update_file_status(input_s3_key, 'error', error_message=str(e))
        ...
        # Mark file as error on BDA failure
        try:
            input_s3_key = event.get('detail', {}).get('input_s3_object', {}).get('name', '')
            if input_s3_key:
                _update_file_status(
                    input_s3_key, 'error',
                    error_message=f"BDA job failed: {event['detail-type']}"
                )
        except Exception as e:
            logger.error(f"Failed to update error status on BDA failure: {e}")
